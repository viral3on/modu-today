...
def calc_signals(sessions, short_map):
    ...
    for code,x in latest.items():
        ...
        # Composite score: rewards unusual participation + positive price action + breakouts/trend.
        score=0.0
        score += min(max(rr["volume_ratio"]-1,0)*10,30)
        score += min(max(rr["value_ratio"]-1,0)*9,27)
        score += min(max(rr["change_pct"],0)*2.2,22)
        score += 8 if rr["new_high_20d"] else 0
        score += 7 if rr["new_high_60d"] else 0
        score += 6 if rr["new_high_120d"] else 0
        score += min(max(rr.get("return_5d",0),0)*0.7,10)
        rr["modu_score"]=r2(min(score,100))
        rr["signal_tags"]=[]
...
def main():
    ...
    payload={
        ...
        "notes":["KRX 공식 OPEN API 일별 데이터 기반","최근 최대 121거래일을 비교해 다음 거래일 참고용 신호를 계산","실시간 호가가 아닌 거래일 단위 스캐너","MODU 관심도는 거래량·거래대금·등락·신고가·추세를 조합한 자체 점수"]
    }
