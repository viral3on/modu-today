...
AVAIL = DATA / "availability_log.json"
...
def record_availability_check(target_day, available, rows, stamp):
    data=read_json(AVAIL, {"checks":[],"first_available":{}})
    if not isinstance(data,dict): data={"checks":[],"first_available":{}}
    checks=data.setdefault("checks",[])
    first=data.setdefault("first_available",{})
    item={
        "target_date":fmt_day(target_day),
        "target_date_raw":target_day,
        "checked_at":stamp.isoformat(timespec="minutes"),
        "checked_at_text":stamp.strftime("%Y.%m.%d %H:%M KST"),
        "available":bool(available),
        "rows":int(rows),
    }
    checks.append(item)
    data["checks"]=checks[-90:]
    if available and target_day not in first:
        first[target_day]=item["checked_at_text"]
        log(f"FIRST AVAILABLE {target_day}: {item['checked_at_text']}")
    data["last_check"]=item
    save_json_atomic(AVAIL,data)
    return data

def scheduled_precheck():
    """예약 실행일 때 2개 핵심 API만 먼저 확인한다.
    미반영이면 121거래일 재수집 없이 종료하고, 이미 갱신된 날이면 후속 예약도 즉시 종료한다.
    반환: (skip_full_build, target_day)
    """
    stamp=datetime.now(KST)
    target=monitor_target_date(stamp)
    log(f"MONITOR TARGET: {target} / CHECKED {stamp.strftime('%Y.%m.%d %H:%M KST')}")
    rows=fetch_core(target)
    available=bool(rows)
    record_availability_check(target,available,len(rows),stamp)
    if not available:
        log(f"MONITOR NOT AVAILABLE: {target} - keep existing scanner data, skip full 121-session build")
        return True,target

    old=read_json(OUT,{})
    old_day=str(old.get("trade_date_raw") or "").replace("-","")
    if old_day==target:
        log(f"MONITOR ALREADY UPDATED: {target} - skip duplicate full build")
        return True,target

    log(f"MONITOR AVAILABLE: {target} rows={len(rows):,} - run full scanner build")
    return False,target
...
def main():
    ...
    if event=="schedule":
        skip,requested_target=scheduled_precheck()
        if skip:
            log("MONITOR CHECK COMPLETE - full build skipped")
            return
    ...
    # 예약 00:30 실행은 전날 장을 목표로 하므로 '오늘 날짜'가 아니라 실제 확인대상과 비교한다.
    requested_raw=requested_target or stamp.strftime("%Y%m%d")
    current_day_missing = latest_day != requested_raw
    if current_day_missing:
        log(f"TARGET DATA NOT AVAILABLE: requested={requested_raw}, selected={latest_day}")

    # 수동 실행에서도 최신 거래일의 최초 확인시각을 남긴다.
    if event!="schedule":
        record_availability_check(latest_day,True,len(latest_rows),stamp)

    payload={
        "source":"KRX OPEN API","trade_date":fmt_day(latest_day),"trade_date_raw":latest_day,
        "requested_date":fmt_day(requested_raw),"current_day_missing":current_day_missing,
        "data_status_text": (f"KRX 목표 거래일 데이터 미반영 · 최근 제공 거래일 {fmt_day(latest_day)} 기준" if current_day_missing else "KRX 목표 거래일 데이터 반영 완료"),
    ...
    if ms.get("total_stocks",0)<100:
        raise RuntimeError(f"수집 종목 수가 비정상적으로 적습니다: {ms.get('total_stocks')}")

    save_json_atomic(OUT,payload)
    save_json_atomic(SNAP,snap)
