...
# Fast preflight: test one approved API call before nationwide collection.
test_region = next((r for r in REGIONS if str(r.get("code")) == "11680"), REGIONS[0])
test_ym = MONTHS[0]
print(f"API TEST START: {test_region['name']} {test_ym}", flush=True)
try:
    test_rows = fetch_one(test_region["code"], test_ym)
    print(f"API TEST OK: {test_region['name']} {test_ym} rows={len(test_rows)}", flush=True)
except Exception as e:
    print(f"API TEST FAIL: {type(e).__name__}: {e}", flush=True)
    print("FATAL: API preflight failed. Nationwide collection was not started; existing JSON files were preserved.", flush=True)
    raise SystemExit(1)
...
print(f"Fetching nationwide apartment trades: regions={len(REGIONS)}, months={len(MONTHS)}, calls={len(jobs)}", flush=True)
