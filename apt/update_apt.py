...
HTTPS_ENDPOINT = "https://apis.data.go.kr/1613000/RTMSDataSvcAptTrade/getRTMSDataSvcAptTrade"
HTTP_ENDPOINT = "http://apis.data.go.kr/1613000/RTMSDataSvcAptTrade/getRTMSDataSvcAptTrade"
...
for label, candidate in [("HTTPS", HTTPS_ENDPOINT), ("HTTP", HTTP_ENDPOINT)]:
    ENDPOINT = candidate
    print(f"API TEST {label} START: {test_region['name']} {test_ym}", flush=True)
    try:
        test_rows = fetch_one(test_region["code"], test_ym)
        print(f"API TEST {label} OK: rows={len(test_rows)}", flush=True)
